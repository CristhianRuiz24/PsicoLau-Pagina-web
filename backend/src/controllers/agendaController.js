...
const { cifrarPaciente, descifrarPaciente } = require('../utils/crypto');
const {
  parseMontoValido,
...
const obtenerCitas = async (req, res) => {
  ...
  const citasDescifradas = citas.map(c => ({
    ...c,
    paciente: c.paciente ? descifrarPaciente(c.paciente) : null
  }));

  res.json({ success: true, data: citasDescifradas });
};
...
const crearCita = async (req, res) => {
  ...
  const citasCreadasDb = await prisma.$transaction(operacionesCitas);

  const citasCreadas = citasCreadasDb.map(cita => ({
    ...cita,
    paciente: cita.paciente ? descifrarPaciente(cita.paciente) : null
  }));
...
const cancelarCita = async (req, res) => {
  ...
  res.json({ 
    ...
    data: {
      ...citaActualizada,
      paciente: citaActualizada.paciente ? descifrarPaciente(citaActualizada.paciente) : null
    }
  });
};
...
const editarCita = async (req, res) => {
  ...
  if (alcance === 'ESTA_Y_SIGUIENTES') {
    ...
    const resultado = await prisma.$transaction(async (tx) => {
      ...
      if (nombreFinal || telefono !== undefined || email !== undefined || enlaceZoom !== undefined || (montoValido !== null && !eraBloqueo && !eraGrupal)) {
        ...
        await tx.paciente.update({
          ...
          data: cifrarPaciente(updatePlano)
        });
      }
    ...
    return res.json({ 
      ...
      data: {
        ...resultado.citaBaseActualizada,
        paciente: resultado.citaBaseActualizada.paciente ? descifrarPaciente(resultado.citaBaseActualizada.paciente) : null
      },
